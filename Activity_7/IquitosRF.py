...
featuresName=['weekofyear','ndvi_ne','ndvi_nw','ndvi_se','ndvi_sw','precipitation_amt_mm',
...
estimators = range(2,15,1)
total_scores = []
for i in estimators:
    regressor = RandomForestRegressor(n_estimators= i, criterion='mae', random_state=0)
    regressor.fit(trainingData, objective)
    y=regressor.predict(trainingData)
    total_scores.append(mean_absolute_error(objective,y))

estimator = total_scores.index(np.amin(total_scores))+2

print 'The best number of estimators is', estimator, 'because he has the lowest mae'
...
regressor = RandomForestRegressor(n_estimators= estimator, criterion='mae', random_state=0)
# Model construction
regressor.fit(trainingData, objective)
# Model prediction
print '\nPrediction:'
...
f =codecs.open("iquitosRF.csv",'wt')
data2=[]
